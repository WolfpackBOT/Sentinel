...
@pytest.fixture
def valid_wolfcoin_address(network='mainnet'):
    return 'tYe8KwyaUu5YswSYmB3q3ryx8XTUu9y7Ui' if (network == 'testnet') else 'EpjStRH8SgA6PjgebtPZqCa9y7hLXP767n'


@pytest.fixture
def invalid_wolfcoin_address(network='mainnet'):
...
def test_valid_wolfcoin_address():
    from wolfcoinlib import is_valid_wolfcoin_address

    main = valid_wolfcoin_address()
    test = valid_wolfcoin_address('testnet')

    assert is_valid_wolfcoin_address(main) is True
    assert is_valid_wolfcoin_address(main, 'mainnet') is True
    assert is_valid_wolfcoin_address(main, 'testnet') is False

    assert is_valid_wolfcoin_address(test) is False
    assert is_valid_wolfcoin_address(test, 'mainnet') is False
    assert is_valid_wolfcoin_address(test, 'testnet') is True


def test_invalid_dash_address():
    from wolfcoinlib import is_valid_wolfcoin_address

    main = invalid_wolfcoin_address()
    test = invalid_wolfcoin_address('testnet')

    assert is_valid_wolfcoin_address(main) is False
    assert is_valid_wolfcoin_address(main, 'mainnet') is False
    assert is_valid_wolfcoin_address(main, 'testnet') is False

    assert is_valid_wolfcoin_address(test) is False
    assert is_valid_wolfcoin_address(test, 'mainnet') is False
    assert is_valid_wolfcoin_address(test, 'testnet') is False
...
def test_deterministic_masternode_elections(current_block_hash, mn_list):
    from wolfcoinlib import elect_mn
...
def test_parse_masternode_status_vin():
    from wolfcoinlib import parse_masternode_status_vin
...
def test_hash_function():
    ...
    hex_hash = "%x" % wolfcoinlib.hashit(sb_data_hex)
