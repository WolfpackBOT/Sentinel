...
import sys
import os
import wolfcoin_config

default_sentinel_config = os.path.normpath(
    os.path.join(os.path.dirname(__file__), '../sentinel.conf')
)
sentinel_config_file = os.environ.get('SENTINEL_CONFIG', default_sentinel_config)
sentinel_cfg = wolfcoin_config.wolfcoinConfig.tokenize(sentinel_config_file)
sentinel_version = "1.1.0"
min_wolfcoind_proto_version_with_sentinel_ping = 70207


def get_wolfcoin_conf():
    home = os.environ.get('HOME')

    wolfcoin_conf = os.path.join(home, ".wolfcoin/wolfcoin.conf")
    if sys.platform == 'darwin':
        wolfcoin_conf = os.path.join(home, "Library/Application Support/wolfcoin/wolfcoin.conf")

    wolfcoin_conf = sentinel_cfg.get('wolfcoin_conf', wolfcoin_conf)

    return wolfcoin_conf
...
wolfcoin_conf = get_wolfcoin_conf()
network = get_network()
db = get_db_conn()
