...
class GovernanceClass(object):
    ...
    def vote(self, wolfcoind, signal, outcome):
        return self.go.vote(wolfcoind, signal, outcome)

    # pass thru to GovernanceObject#voted_on
    def voted_on(self, **kwargs):
        return self.go.voted_on(**kwargs)

    def vote_validity(self, wolfcoind):
        if self.is_valid():
            printdbg("Voting valid! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(wolfcoind, models.VoteSignals.valid, models.VoteOutcomes.yes)
        else:
            printdbg("Voting INVALID! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(wolfcoind, models.VoteSignals.valid, models.VoteOutcomes.no)
    ...
    def list(self):
        ...
        # return a dict similar to wolfcoind "gobject list" output
        return {self.object_hash: dikt}

    def get_submit_command(self):
        import wolfcoinlib as egilib
        obj_data = egilib.SHIM_serialise_for_wolfcoind(self.serialise())
    ...
    def wolfcoind_serialise(self):
        import wolfcoinlib
        return wolfcoinlib.SHIM_serialise_for_wolfcoind(self.serialise())
