...
def is_valid_wolfcoin_address(address, network='mainnet'):
    ...
    wolfcoin_version = 140 if network == 'testnet' else 76
    ...
    # except:
    #     # rescue from exception, not a valid wolfcoin address
    #     return False

    # if (address_version != wolfcoin_version):
    #     return False
...
# shims 'til we can fix the wolfcoind side
def SHIM_serialise_for_wolfcoind(sentinel_hex):
    from models import wolfcoinD_GOVOBJ_TYPES
    # unpack
    ...
    # add 'type' attribute
    obj[1]['type'] = wolfcoinD_GOVOBJ_TYPES[govtype]

    # superblock => "trigger" in wolfcoind
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # wolfcoind expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    wolfcoind_hex = serialise(obj)
    return wolfcoind_hex


# shims 'til we can fix the wolfcoind side
def SHIM_deserialise_from_wolfcoind(wolfcoind_hex):
    # unpack
    obj = deserialise(wolfcoind_hex)
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('wolfcoin.conf')
