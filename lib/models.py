...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, wolfcoind):
        golist = wolfcoind.rpc_command('gobject', 'list')
        ...
        try:
            ...
            for item in golist.values():
                (go, subobj) = self.import_gobject_from_wolfcoind(wolfcoind, item)
    ...
    @classmethod
    def import_gobject_from_wolfcoind(self, wolfcoind, rec):
        import wolfcoinlib
        import decimal
        import inflection
        ...
        # shim/wolfcoind conversion
        object_hex = wolfcoinlib.SHIM_deserialise_from_wolfcoind(object_hex)
        objects = wolfcoinlib.deserialise(object_hex)
        subobj = None
        ...
        # exclude any invalid model data from wolfcoind...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from wolfcoind, with every run
        ...
        # get/create, then sync payment amounts, etc. from wolfcoind - wolfcoind is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(wolfcoind)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from wolfcoind! %s" % e)
            govobj.vote_delete(wolfcoind)
            return (govobj, None)
    ...
    def vote_delete(self, wolfcoind):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(wolfcoind, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, wolfcoind, signal, outcome):
        import wolfcoinlib as egilib
        ...
        output = wolfcoind.rpc_command(*vote_command)
        ...
        if voted:
            ...
            self.sync_network_vote(wolfcoind, signal)

    def sync_network_vote(self, wolfcoind, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = wolfcoind.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = wolfcoinD_GOVOBJ_TYPES['proposal']
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 wolfcoin addr, non-multisig
            if not egilib.is_valid_wolfcoin_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid wolfcoin address for network [%s], returning False" % (self.payment_address, config.network))
                return False
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (wolfcoinlib.blocks_to_seconds(superblockcycle) / 2) +
    ...
    def is_deletable(self):
        ...
        # TBD (item moved to external storage/wolfcoinDrive, etc.)
        return False
    ...
    def get_prepare_command(self):
        import wolfcoinlib
        obj_data = wolfcoinlib.SHIM_serialise_for_wolfcoind(self.serialise())
    ...
    def prepare(self, wolfcoind):
        try:
            object_hash = wolfcoind.rpc_command(*self.get_prepare_command())
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = wolfcoinD_GOVOBJ_TYPES['superblock']
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not wolfcoinlib.is_valid_wolfcoin_address(addr, config.network):
    ...
    def hash(self):
        import wolfcoinlib
        return wolfcoinlib.hashit(self.serialise())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    govobj_type = wolfcoinD_GOVOBJ_TYPES['watchdog']
    only_masternode_can_submit = True

    @classmethod
    def active(self, wolfcoind):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - wolfcoind.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, wolfcoind):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - wolfcoind.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, wolfcoind):
        now = int(time.time())
        return (self.created_at < (now - wolfcoind.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, wolfcoind):
        if self.is_expired(wolfcoind):
            return False

        return True

    def is_deletable(self, wolfcoind):
        if self.is_expired(wolfcoind):
            return True
