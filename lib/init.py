...
def has_wolfcoin_conf():
    import config
    import io

    valid_wolfcoin_conf = False

    # ensure wolfcoin_conf exists & readable
    #
    # if not, print a message stating that wolfcoin Core must be installed and
    # configured, including JSONRPC access in wolfcoin.conf
    try:
        f = io.open(config.wolfcoin_conf)
        valid_wolfcoin_conf = True
    except IOError as e:
        print(e)

    return valid_wolfcoin_conf
...
def main():
    ...
    if not has_wolfcoin_conf():
        print("wolfcooin must be installed and configured, including JSONRPC access in wolfcoin.conf")
        sys.exit(1)
