...
class wolfcoinDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some wolfcoind methods
    ...
    @classmethod
    def from_wolfcoin_conf(self, wolfcoin_dot_conf):
        from wolfcoin_config import wolfcoinConfig
        config_text = wolfcoinConfig.slurp_config_file(wolfcoin_dot_conf)
        creds = wolfcoinConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = wolfcoinlib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = wolfcoinlib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = wolfcoinlib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_wolfcoind_proto_version_with_sentinel_ping)
